    ...
    def insert(
        ...
        Args:
            ...
            num_threads: Number of worker threads used to upload the item
                ...
                ``1`` to upload sequentially. A value above ``32`` is accepted
                and clamped to it rather than rejected, as on the read path.
                It also sizes how much the upload holds: request bodies are
                queued uncompressed, up to ``2 * num_threads`` of them at
                ``MAX_BATCH_SIZE_MB`` each -- about 40 MB at the default ``4``
                and about 320 MB at ``32``. Raise it for throughput, lower it
                where memory is tight. All batches land in a single dataset
                version. If a batch fails the call raises, and the
